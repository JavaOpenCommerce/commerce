...
class ItemIndexingService {
  ...
  ItemIndexingService(Vertx vertx, ElasticAddress address, ItemQueryRepository itemQueryRepository,
    ...
    this.client = WebClient.create(vertx,
        new WebClientOptions().setDefaultPort(address.getPort()).setDefaultHost(address.getHost()));
  }
  ...
  private void refreshIndex() {
    client.delete("/items").send(ar -> {
      if (ar.succeeded()) {
        log.info("'Item' index successfully cleaned: {}", ar.result().bodyAsString());
        updateIndexSettings();
      } else {
        log.info("Problems during 'item' index cleanup: {}", ar.result().bodyAsJsonObject());
      }
    });
  }
  ...
  private void updateIndexSettings() {
    ...
    client.put("/items").putHeader("Content-Type", APPLICATION_JSON)
  ...
  private void sendItem(SearchItem item) {
    client.put("/items/_doc/" + item.id())
        ...
        .putHeader("Content-Type", APPLICATION_JSON).sendJson(item, ar -> {
  ...
  private List<SearchItem> getSearchItems() {
    List<SearchItem> searchItems = itemQueryRepository.getAllFullItems().stream()
        .map(SearchItemConverter::convertToSearchItem).collect(toList());
  ...
  private void enrichCategories(List<SearchItem> items) {
    ...
    for (SearchItem item : items) {
      ...
      item.categoryIds().forEach(id -> ids.addAll(
          categoryEnricher.findAllCategoryIdsForId(UUID.fromString(id)).stream().map(UUID::toString)
              .toList()));
