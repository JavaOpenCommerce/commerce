...
import com.example.opencommerce.domain.catalog.Category.CategorySnapshot;
...
class CatalogTest {
    ...
    void shouldFindAllSubcategoriesOfEnduroCategory() {
        // given
        CategoryId id = ENDURO.toSnapshot()
                .id();
        ...
        assertThat(subcategories).hasSize(2)
                .contains(MOUNTAIN_BIKES.toSnapshot()
                        .id(), CATALOG_ROOT.toSnapshot()
                        .id());
    ...
    void shouldFindAllSubcategoriesOfGravelsSCategory() {
        // given
        CategoryId id = GRAVELS.toSnapshot()
                .id();
        ...
        // then
        assertThat(subcategories).hasSize(2)
                .contains(ROAD_BIKES.toSnapshot()
                        .id(), CATALOG_ROOT.toSnapshot()
                        .id());
    ...
    void shouldAddNewCategoryAsSubcategoryOfEnduroAndThenRemoveIt() {
        ...
        // when
        CATALOG_ROOT.addChildToCategoryWithId(ENDURO.toSnapshot()
                        .id(), "All Mountain",
                "Cross Country approach");

        // then
        assertThat(ENDURO.getChildren()).hasSize(1);
        Category allMountainCategory = ENDURO.getChildren()
                .stream()
                .toList()
                .get(0);
        assertThat(allMountainCategory).extracting(Category::toSnapshot)
                ...
                .isEqualTo("All Mountain");

        // when
        CATALOG_ROOT.removeCategoryWithId(allMountainCategory.toSnapshot()
                .id());
