...
public class CardValidationTest {
  ...
  void shouldNotPassValidationBecauseOfNonMatchingGrossPrice() {
    ...
    SoftAssertions.assertSoftly(softly -> {
      softly.assertThat(validationException.getDerivativeExceptions()).hasSize(2);
      softly.assertThat(validationException.getDerivativeExceptions().get(0)).isInstanceOf(
          OrderValueNotMatchingValidationException.class);
      softly.assertThat(validationException.getDerivativeExceptions().get(0).getMessage())
          .contains("value=20.00");
      softly.assertThat(validationException.getDerivativeExceptions().get(1)).isInstanceOf(
          OrderValueNotMatchingValidationException.class);
      softly.assertThat(validationException.getDerivativeExceptions().get(1).getMessage())
  ...
  void shouldNotPassValidationBecauseOfExceededStock() {
    ...
    // then
    SoftAssertions.assertSoftly(softly -> {
      softly.assertThat(validationException.getDerivativeExceptions()).hasSize(1);
      softly.assertThat(validationException.getDerivativeExceptions().get(0)).isInstanceOf(
  ...
  void shouldNotPassValidationBecauseOfExceededStockAndPricingMismatches() {
    ...
    // then
    SoftAssertions.assertSoftly(softly -> {
      softly.assertThat(validationException.getDerivativeExceptions()).hasSize(3);
    });
  }
}
