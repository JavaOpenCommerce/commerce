...
class ItemDtoFactory {
  ...
  ItemDto productToDto(ItemEntity itemEntity) {
    return ItemDto.builder().id(itemEntity.getId()).stock(itemEntity.getStock())
        .valueGross(itemEntity.getValueGross()).vat(itemEntity.getVat()).name(itemEntity.getName())
        .image(toDto(itemEntity.getImage())).build();
  }
  ...
  FullItemDto toFullProductDto(ItemEntity itemEntity) {
    ...
    return FullItemDto.builder().id(itemEntity.getId()).stock(itemEntity.getStock())
        .valueGross(itemEntity.getValueGross()).vat(itemEntity.getVat())
        .description(details.getDescription()).name(itemEntity.getName())
        .mainImage(toDto(itemEntity.getImage()))
        .categoryIds(itemEntity.getCategoryIds())
        .producer(toDto(producer))
        .additionalImages(details.getAdditionalImages().stream().map(this::toDto).toList()).build();
  }
